...
module.exports = function(util, listener){
  ...
  const checkerReg = /(\.)/i;
  let list = fs.readdirSync(`${currentPath}/src/apps`);

  list = list.filter(function(val) {
    return !checkerReg.test(val);
  });
  ...
  return new MultiSelect({
    name: "value",
    message: "Choice the project [micro] package for development",
    choices: list
  })
    .run()
    .then(entry=> {
      if (entry.length) {
        ...
        _each(entry, function(entryName){
          presetIndexFile += `export { default as ${entryName} } from '@${entryName}/entry';\n`;
        });

        fs.writeFileSync(`${currentPath}/src/apps/index.ts`, presetIndexFile);
      ...
      return error("must choice less than one [micro] package entry for devServer!");
    })
    .catch(console.error);
};
