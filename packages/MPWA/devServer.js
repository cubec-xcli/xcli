    ...
    .then(entry=> {
      if (entry.length) {
        ...
        _each(entry, function(entryName){
          presetIndexFile += `import ${entryName} from "./${entryName}";\n`;
        });

        presetIndexFile+=`export { ${entry.toString()} }\n`;
        presetIndexFile+=`export default { ${entry.toString()} }\n`;

        fs.writeFileSync(`${currentPath}/src/index.js`, presetIndexFile);
